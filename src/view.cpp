...
//Box2Db uses meters, seconds, radians
//Qt     uses pixels, seconds, angles
//convert it : 32px = 1m, s=s, angle = radians/pi*180angles
...
b2Body *View::createEntity(int pos_x, int pos_y, int size_x, int size_y, uintptr_t type)
{
    ...
    else
    {
        ...
        b2shape.m_radius = pixel_to_meters(25.0/2.0);
        fixtureDef.shape = &b2shape;
...
void View::keyReleaseEvent(QKeyEvent *event)
{
    if(!event->isAutoRepeat())
    {
        ...
        {
            m_world.SetGravity(b2Vec2(-9.8f, 0));
        }
        ...
        {
            m_world.SetGravity(b2Vec2(0, 9.8f));
        }
